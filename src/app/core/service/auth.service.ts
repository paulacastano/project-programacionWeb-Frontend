...
export class AuthService {
  ...
  getRoleInfoByToken(): { roleId: number; roleName: string } | undefined {
    // Es el nombre del método. Su objetivo es devolver información del rol del usuario (ID y nombre) que ha sido autenticado.
    ...
    try {
      ...
      if (roleId === 1) {
        // Evalúa el roleId: Si es 1, se considera Administrador. Si es 2, se considera Usuario. Si no es ninguno de esos dos, retorna undefined porque no reconoce el rol.
      ...
      // Si algo sale mal (por ejemplo, el token no existe o no se puede decodificar), se captura el error. Se imprime el error en la consola y el método retorna undefined.
      console.error(error);
      return undefined;
    }
  }
